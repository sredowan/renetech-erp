...
function CheckoutForm() {
  ...
  const formatSchedule = (scheduleStr) => {
    ...
    try {
      // Handle both pre-parsed objects (from Laravel array cast) and JSON strings (from Node.js)
      const sch = typeof scheduleStr === 'object' ? scheduleStr : JSON.parse(scheduleStr);
      ...
      // If it's a plain string description, return it; if it's an object, stringify it
      if (typeof scheduleStr === 'object') return JSON.stringify(scheduleStr);
      return scheduleStr;
    }
    // Final fallback: if it's an object that didn't match above, stringify it
    if (typeof scheduleStr === 'object') return JSON.stringify(scheduleStr);
    return scheduleStr;
  };
