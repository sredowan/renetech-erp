...
const Students = () => {
  ...
  const handleAddStudent = async (submissionData) => {
    ...
    try {
      ...
      const newStudent = response.data.student || response.data.Student;
      if (newStudent) {
        setStudents([newStudent, ...students]);
      }
      setIsAddModalOpen(false);
      if (response.data.invoice || response.data.Invoice) {
  ...
  return (
    <div className="students-page" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      {/* Header — desktop only */}
