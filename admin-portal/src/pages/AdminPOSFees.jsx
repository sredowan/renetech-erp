...
const POSFees = () => {
  ...
  const filteredTransactions = transactions.filter(r => {
    ...
    const name = r.source === 'manual' ? (r.Invoice?.Customer?.name || r.Invoice?.customer_name || 'Customer') : (r.Enrollment?.Student?.User?.name || 'Unknown');
    const desc = r.source === 'manual' ? (r.Invoice?.IncomeCategory?.name || 'Custom Income') : (r.Enrollment?.Batch?.Course?.title || 'Tuition Fee');
    const matchesSearch = !searchQuery ||
      `trx-${r.id}`.includes(sLower) ||
      name.toLowerCase().includes(sLower) ||
      desc.toLowerCase().includes(sLower) ||
      r.amount.toString().includes(sLower);
  ...
  const filteredPending = pendingInvoices.filter(inv => {
    const sLower = searchQuery.toLowerCase();
    const name = getInvoiceStudentName(inv).toLowerCase();
    const desc = (inv.invoice_type === 'custom' ? (inv.IncomeCategory?.name || 'Manual Invoice') : (inv.Enrollment?.Batch?.Course?.title || 'Tuition Fee')).toLowerCase();
    const matchesSearch = !searchQuery ||
      (inv.invoice_no && inv.invoice_no.toLowerCase().includes(sLower)) ||
      name.includes(sLower) ||
      desc.includes(sLower) ||
      inv.amount.toString().includes(sLower);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div className="g65">
        <div className="sc">
          {activeTab === 'recent' && (
            <>
              <div className="sc-head" style={{ flexWrap: 'wrap', gap: '10px' }}>
                ...
                <div className="row" style={{ gap: '6px', flexWrap: 'wrap' }}>
                  <input
                    ...
                    style={{ padding: '5px 10px', fontSize: '11px', width: '200px' }}
                    value={searchQuery}
              ...
              <div className="st-wrap" style={{ overflowX: 'auto', width: '100%' }}>
              ...
              {txTotalPages > 1 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 15px', borderTop: '1px solid var(--border)', flexWrap: 'wrap', gap: '10px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                    <button onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="btn-ghost" style={{ padding: '4px 8px', fontSize: '11px' }}>Prev</button>
                    {[...Array(txTotalPages)].map((_, i) => (
                      <button key={i} onClick={() => setCurrentPage(i + 1)} className={currentPage === i + 1 ? 'btn-stitch' : 'btn-ghost'} style={{ padding: '4px 8px', fontSize: '11px', background: currentPage === i + 1 ? '#275fa7' : 'transparent', color: currentPage === i + 1 ? '#fff' : 'var(--text)' }}>
                        {i + 1}
                      </button>
                    ))}
                    <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, txTotalPages))} disabled={currentPage === txTotalPages} className="btn-ghost" style={{ padding: '4px 8px', fontSize: '11px' }}>Next</button>
                  </div>
                </div>
              )}
            </>
          ...
          {activeTab === 'pending' && (
            <>
              ...
              {pendingTotalPages > 1 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 15px', borderTop: '1px solid var(--border)', flexWrap: 'wrap', gap: '10px' }}>
                  <span style={{ fontSize: '11px', color: 'var(--text-dim)' }}>
                    Showing {(currentPage - 1) * itemsPerPage + 1} to {Math.min(currentPage * itemsPerPage, filteredPending.length)} of {filteredPending.length} entries
                  </span>
                  <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                    <button onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="btn-ghost" style={{ padding: '4px 8px', fontSize: '11px' }}>Prev</button>
                    ...
                    <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, pendingTotalPages))} disabled={currentPage === pendingTotalPages} className="btn-ghost" style={{ padding: '4px 8px', fontSize: '11px' }}>Next</button>
                  </div>
                </div>
              )}
            </>
