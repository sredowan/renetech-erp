...
export const generateReceiptHtml = async (tx) => {
  ...
  const branch = tx.Branch?.name || tx.branch?.name || tx.branch_name || 'Head Office';
...
export const generateVoucherHtml = async (expense) => {
  ...
  const branch = expense.Branch?.name || expense.branch?.name || expense.branch_name || 'Head Office';
